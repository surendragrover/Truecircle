...
class _CBTCenterPageState extends State<CBTCenterPage> {
  ...
  Future<void> _ensure() async {
    ...
    _showHindiArticle = prefs.get('articles_show_hindi', defaultValue: false) as bool;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: !_init ? const Center(child: CircularProgressIndicator()) : SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionTitle('Screening Tests'),
            _cardButton('PHQ‑9 Depression (Demo)', Icons.sentiment_very_dissatisfied, _runPhq9Demo),
            _cardButton('GAD‑7 Anxiety (Demo)', Icons.sentiment_neutral, _runGad7Demo),
            const SizedBox(height:24),
            _sectionTitle('CBT Thought Diary'),
            _cardButton('Add Thought Record', Icons.note_add, _addThoughtRecord),
  ...
  Widget _cardButton(String label, IconData icon, VoidCallback onTap) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors:[Color(0xFF00796B), Color(0xFF00897B)]),
  ...
  Future<void> _runPhq9Demo() async {
    final answers = { for (int i=1;i<=9;i++) 'q$i': (i%4) }; // mock rotating scores
    ...
    final r = CBTAssessmentResult(
      ...
      severityBand: band,
      localizedSummaryEn: 'PHQ‑9 Score $total ($band). Practice a grounding exercise & track mood.',
      localizedSummaryHi: 'PHQ‑9 स्कोर $total ($band). ग्राउंडिंग अभ्यास करें और मूड ट्रैक करें।',
    );
  ...
  Future<void> _runGad7Demo() async {
    ...
    final r = CBTAssessmentResult(
      ...
      timestamp: DateTime.now(),
      severityBand: band,
      localizedSummaryEn: 'GAD‑7 Score $total ($band). Try a breathing session & reframe worry.',
      localizedSummaryHi: 'GAD‑7 स्कोर $total ($band). एक श्वास सत्र करें और चिंता को रिफ्रेम करें।',
    );
    await CBTService.instance.saveAssessment(r);
    if (!mounted) return; setState((){});
  }
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        const Expanded(child: Text('New Thought Record', style: TextStyle(fontSize:18,fontWeight: FontWeight.bold))),
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(labelText: 'Situation / Context'),
                    ),
                    TextField(
                      controller: thoughtCtrl,
                      decoration: const InputDecoration(labelText: 'Automatic Thought'),
                    ),
                    TextField(
                      controller: emotionCtrl,
                      decoration: const InputDecoration(labelText: 'Primary Emotion (e.g., Sad, Anxious)'),
                    ),
                    const SizedBox(height:12),
                    Text('Intensity Before: $intensityBefore%', style: const TextStyle(fontSize:12,fontWeight: FontWeight.w600)),
                    Slider(value: intensityBefore.toDouble(), min:0,max:100, divisions:20,
                      onChanged: (v){ setM(()=> intensityBefore = v.round()); },
                    ),
                    const SizedBox(height:4),
                    DropdownButtonFormField<Map<String,String>>(
                      initialValue: selectedDistortion,
                      decoration: const InputDecoration(labelText: 'Cognitive Distortion'),
                      items: distortions.map((d)=> DropdownMenuItem(
                        value: d,
                        child: Text(d['title']!),
                      )).toList(),
                      onChanged: (v){ setM(()=> selectedDistortion = v); },
                    ),
                    const SizedBox(height:12),
                    Wrap(
                      spacing:6,
                      runSpacing:4,
                      children: techniques.map((t)=> ActionChip(
                    ...
                    TextField(
                      ...
                      maxLines: 4,
                      decoration: const InputDecoration(labelText: 'Rational / Balanced Response'),
                    ),
                    const SizedBox(height:12),
                    Text('Intensity After: $intensityAfter%', style: const TextStyle(fontSize:12,fontWeight: FontWeight.w600)),
                    Slider(value: intensityAfter.toDouble(), min:0,max:100, divisions:20,
                      onChanged: (v){ setM(()=> intensityAfter = v.round()); },
                    ),
                    const SizedBox(height:12),
                    Row(
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            ...
                            onPressed: () async {
                              if (thoughtCtrl.text.trim().isEmpty || emotionCtrl.text.trim().isEmpty || selectedDistortion == null) {
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill thought, emotion & distortion.')));
                                return;
                              }
                              final tr = CBTThoughtRecord(
                                ...
                                cognitiveDistortion: selectedDistortion!['title']!,
                                rationalResponse: rationalCtrl.text.trim().isEmpty ? '—' : rationalCtrl.text.trim(),
                    ...
                    const SizedBox(height:4),
                    const Text('Tip: Use technique chips to scaffold a balanced response.', style: TextStyle(fontSize:11, fontStyle: FontStyle.italic)),
                  ],
                ),
              );
            },
  ...
  Future<void> _addCopingCard() async {
    final c = CopingCard(
      ...
      adaptiveBelief: 'Progress over perfection – partial success still moves me forward.',
  ...
  Widget _buildThoughtRecordsList() {
    ...
    return Column(
      children: list.take(5).map((tr) => ListTile(
        contentPadding: const EdgeInsets.symmetric(horizontal:4),
        leading: const Icon(Icons.note, size:20),
        title: Text(tr.automaticThought, maxLines:1, overflow: TextOverflow.ellipsis),
        subtitle: Text('${tr.emotion} ${tr.emotionIntensityBefore}%→${tr.emotionIntensityAfter}%', style: const TextStyle(fontSize:11)),
      )).toList(),
    );
  }
  ...
  Widget _buildCopingCardsList() {
    ...
    if (list.isEmpty) return const SizedBox.shrink();
    return Column(
      children: list.take(5).map((c) => ListTile(
        contentPadding: const EdgeInsets.symmetric(horizontal:4),
        leading: const Icon(Icons.memory, size:20),
        title: Text(c.unhelpfulBelief, maxLines:1, overflow: TextOverflow.ellipsis),
        subtitle: Text(c.adaptiveBelief, maxLines:1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize:11)),
      )).toList(),
    );
  }
    ...
    return Column(
      children: [
        ExpansionTile(
          ...
          children: cognitive.map((m)=> ListTile(
            title: Text(m['title']!),
            subtitle: Text(m['en']!, maxLines:2, overflow: TextOverflow.ellipsis),
            onTap: () => _showDetailSheet(m['title']!, m['en']!, m['hi']!),
          )).toList(),
        ),
        ExpansionTile(
          leading: const Icon(Icons.fitness_center),
            title: const Text('Behavioral Techniques'),
            children: behavioral.map((m)=> ListTile(
              title: Text(m['title']!),
              subtitle: Text(m['en']!, maxLines:2, overflow: TextOverflow.ellipsis),
              onTap: () => _showDetailSheet(m['title']!, m['en']!, m['hi']!),
            )).toList(),
        ...
        ExpansionTile(
          ...
          children: distortions.map((m)=> ListTile(
            title: Text(m['title']!),
            subtitle: Text(m['en']!, maxLines:2, overflow: TextOverflow.ellipsis),
            onTap: () => _showDetailSheet(m['title']!, m['en']!, m['hi']!),
          )).toList(),
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height:12),
            Container(
              decoration: BoxDecoration(color: Colors.teal.withValues(alpha:0.07), borderRadius: BorderRadius.circular(12)),
              padding: const EdgeInsets.all(12),
              child: Text(hi, style: const TextStyle(fontSize:13, fontStyle: FontStyle.italic)),
            ),
            const SizedBox(height:12),
            Align(
  ...
  Widget _buildArticlesSection() {
    ...
    return Column(
      children: [
        ...
        const SizedBox(height:12),
        if (filtered.isEmpty)
          const Text('No matches', style: TextStyle(fontSize:12, fontStyle: FontStyle.italic)),
        ...filtered.map((a) => Card(
          elevation: 1,
          ...
          child: ListTile(
            ...
            subtitle: Text(a['summary']!, maxLines:2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize:12)),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(a['author']!, style: const TextStyle(fontSize:12, fontStyle: FontStyle.italic, color: Colors.teal)),
                const SizedBox(height:12),
                Text(a['body']!, style: const TextStyle(fontSize:14, height:1.35)),
    ...
    return Column(
      ...
      children: [
        Wrap(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              onPressed: () async {
                ...
                if (orchestrator.isStarted) {
                  final existing = Map<String,String>.from(orchestrator.featureInsights.value);
                  existing['articles'] = 'Articles: ${stats['total']} (Fav ${stats['favorites']})';
                  orchestrator.featureInsights.value = existing;
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Article stats injected into insights')));
                  }
                } else {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Orchestrator not started yet')));
                  }
                }
              },
            ),
            ElevatedButton.icon(
              ...
              onPressed: () async {
                await ArticleShareService.instance.migrateStaticArticlesIfNeeded();
                if (mounted) setState((){});
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Static articles migrated (if any).')));
                }
              },
            ),
            ElevatedButton.icon(
              ...
              onPressed: () async {
                ...
                if (mounted) setState((){});
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Static article versions synced.')));
                }
              },
            ),
          ],
        ...
        TextField(
          decoration: InputDecoration(
            prefixIcon: const Icon(Icons.search),
            suffixIcon: _importedSearch.isNotEmpty ? IconButton(icon: const Icon(Icons.clear), onPressed: () => setState(()=> _importedSearch = '')) : null,
            ...
            Expanded(
              child: Row(
                children: [
                  ...
                  const Flexible(child: Text('Fav first', style: TextStyle(fontSize:11)))
                ],
              ),
            ),
          ],
        ...
        FutureBuilder(
          ...
          builder: (ctx, snap) {
            if (snap.connectionState == ConnectionState.waiting) {
              return const Padding(padding: EdgeInsets.all(8.0), child: LinearProgressIndicator(minHeight: 3));
            }
            var list = snap.data ?? [];
            if (_sortFavoritesFirst) {
              list.sort((a,b){
                if (a.favorite == b.favorite) return b.createdAt.compareTo(a.createdAt);
                return a.favorite ? -1 : 1;
              });
            }
            if (list.isEmpty) {
              return const Text('No matching imported articles.', style: TextStyle(fontSize:12, fontStyle: FontStyle.italic));
            }
            return Column(
              children: list.map((a) => ListTile(
                leading: Icon(a.favorite ? Icons.star : Icons.star_border, color: a.favorite ? Colors.amber : null),
                title: Text(a.title, maxLines:1, overflow: TextOverflow.ellipsis),
                subtitle: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('${a.author}  •  ${a.sourceEmail}', style: const TextStyle(fontSize:11)),
                    if (a.tags.isNotEmpty) Padding(
                      padding: const EdgeInsets.only(top:2),
                      child: Wrap(
                        spacing: 4,
                        runSpacing: -6,
                        children: a.tags.take(6).map((t)=> Chip(label: Text(t, style: const TextStyle(fontSize:10)), visualDensity: VisualDensity.compact, materialTapTargetSize: MaterialTapTargetSize.shrinkWrap)).toList(),
                      ),
                    ),
                  ],
                ),
                trailing: IconButton(
                  icon: Icon(a.favorite ? Icons.bookmark_remove : Icons.bookmark_add_outlined, size:20),
                  onPressed: () async {
                    await ArticleShareService.instance.toggleFavorite(a.id);
                    if (mounted) setState((){});
                  },
                ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (!mounted) return;
              if (article == null) {
                messenger.showSnackBar(const SnackBar(content: Text('Invalid or used token')));
              } else {
                messenger.showSnackBar(SnackBar(content: Text('Imported: ${article.title}')));
                setState((){});
              }
              navigator.pop();
            },
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(
          controller: ctrl,
          maxLines: 4,
          decoration: const InputDecoration(hintText: 'Enter tokens separated by space or newline'),
        ),
        actions: [
          TextButton(onPressed: ()=> Navigator.pop(context), child: const Text('Cancel')),
          ElevatedButton(
            onPressed: () async {
              ...
              final imported = await ArticleShareService.instance.batchRedeem(parts);
              if (!mounted) return;
              navigator.pop();
              setState((){});
              messenger.showSnackBar(SnackBar(content: Text('Imported ${imported.length} / ${parts.length} tokens')));
            },
  ...
  Future<void> loadPrefs(StateSetter setM) async {
      ...
      autoHindi = settingsBox!.get('auto_hindi_translate_articles', defaultValue: false) as bool;
      if (mounted) setM((){});
    }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: StatefulBuilder(
          builder: (ctx,setM){
            if (settingsBox == null) { loadPrefs(setM); }
            return SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(children:[const Expanded(child: Text('New Article', style: TextStyle(fontSize:18,fontWeight: FontWeight.bold))), IconButton(icon: const Icon(Icons.close), onPressed: ()=> Navigator.of(ctx).pop())]),
                TextField(controller: titleCtrl, decoration: const InputDecoration(labelText: 'Title')),
                TextField(controller: authorCtrl, decoration: const InputDecoration(labelText: 'Author')),
                TextField(controller: emailCtrl, decoration: const InputDecoration(labelText: 'Source Email')),
                const SizedBox(height:8),
                TextField(controller: bodyCtrl, maxLines: 6, decoration: const InputDecoration(labelText: 'Body / Content (English)')),
                const SizedBox(height:8),
                Row(
                  children: [
                    Expanded(child: TextField(controller: bodyHiCtrl, maxLines: 4, decoration: const InputDecoration(labelText: 'Body (Hindi Optional / Auto)'))),
                    ...
                    Column(
                      children: [
                        Switch(
                          ...
                          onChanged: (v) async {
                            setM(()=> autoHindi = v);
                            if (settingsBox != null) settingsBox!.put('auto_hindi_translate_articles', v);
                            if (v) {
                              // attempt immediate translation if Hindi empty
                              if (bodyHiCtrl.text.trim().isEmpty && bodyCtrl.text.trim().isNotEmpty) {
                                setM(()=> translating = true);
                                final t = await OfflineArticleAutoTranslator.englishToHindi(bodyCtrl.text.trim());
                ...
                if (translating) const Padding(
                  padding: EdgeInsets.only(top:4),
                  child: LinearProgressIndicator(minHeight: 3),
                ),
                if (autoHindi) Padding(
                  padding: const EdgeInsets.only(top:4),
                  child: Text('Offline dictionary translation – may be partial.', style: TextStyle(fontSize:10, fontStyle: FontStyle.italic, color: Colors.teal.shade700)),
                ),
                ...
                Align(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () async {
                      final navigator = capturedNavigator; // pre-captured before await
                      final messenger = capturedMessenger;
                      if (titleCtrl.text.trim().isEmpty || bodyCtrl.text.trim().length < 10) {
                        if (!mounted) return; messenger.showSnackBar(const SnackBar(content: Text('Title & longer body required')));
                        return;
                      }
                      ...
                      final token = await ArticleShareService.instance.createArticleToken(
                        title: titleCtrl.text.trim(),
                        author: authorCtrl.text.trim(),
                        body: bodyCtrl.text.trim(),
                        sourceEmail: emailCtrl.text.trim(),
                        bodyHi: bodyHiCtrl.text.trim().isEmpty ? null : bodyHiCtrl.text.trim(),
                      );
                      if (!mounted) return;
                      navigator.pop();
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height:4),
            SelectableText('Token: $token', style: const TextStyle(fontSize:16,fontWeight: FontWeight.bold, letterSpacing:1.2)),
            const SizedBox(height:8),
            const Text('Share this token manually (email / message). Recipient can redeem locally. ', style: TextStyle(fontSize:11)),
          ],
        ),
        actions: [
          TextButton(
            onPressed: () {
              ...
              navigator.pop();
              messenger.showSnackBar(const SnackBar(content: Text('Token copied to clipboard')));
            },
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: Text('Source: ${a.sourceEmail}', style: const TextStyle(fontSize:11, fontStyle: FontStyle.italic, color: Colors.teal))),
                    TextButton(
                      onPressed: () async {
                      ...
                      child: Text(_showHindiArticle ? 'EN' : 'HI', style: const TextStyle(fontSize:12)),
                    ),
                    IconButton(
                      tooltip: 'Export',
                      icon: const Icon(Icons.upload_file, size:20),
                      onPressed: () {
                        final serialized = ArticleShareService.instance.serializeForExport(a, includeHindi: _showHindiArticle);
                        Clipboard.setData(ClipboardData(text: serialized));
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Article copied for export')));
                      },
                    )
                  ],
                ),
                const SizedBox(height:10),
                Text(_showHindiArticle ? (a.bodyHi ?? a.body) : a.body, style: const TextStyle(fontSize:14, height:1.35)),
                const SizedBox(height:18),
                _articleTechniquesSection({'title': a.title, 'en': a.body, 'hi': a.bodyHi ?? a.body}),
                const SizedBox(height:24),
              ],
            ),
          ),
  ...
  Widget _articleTechniquesSection(Map<String,String> article) {
    ...
    final behavioral = CBTEducationContent.behavioralTechniques.take(2).toList();
    final combined = [...cognitive, ...behavioral];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Related CBT Techniques', style: TextStyle(fontSize:15,fontWeight: FontWeight.w600)),
        const SizedBox(height:8),
        Wrap(
          spacing: 6,
          runSpacing: 6,
          children: combined.map((t) => FilterChip(
            label: Text(t['title']!, style: const TextStyle(fontSize:11)),
            selected: false,
            onSelected: (_) => _showDetailSheet(t['title']!, t['en']!, t['hi']!),
          )).toList(),
        ),
      ],
    );
  ...
  Widget _buildInsightTimestamp() {
    ...
    return Text('Insights updated: ${ts.toLocal().toIso8601String()}', style: const TextStyle(fontSize:10, fontStyle: FontStyle.italic));
  }
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      ...
                      action: SnackBarAction(label: 'UNDO', onPressed: () async {
                        ...
                        final box = await Hive.openBox<SharedArticle>('shared_articles');
