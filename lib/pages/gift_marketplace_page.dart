...
class _GiftMarketplacePageState extends State<GiftMarketplacePage> {
  ...
  MetricsSnapshot? get _latestMetrics =>
      MetricsAggregatorService.instance.snapshotNotifier.value;
  Map<String, String> get _aiInsights =>
      AIOrchestratorService().featureInsights.value;
  ...
  Future<void> _loadAISuggestions() async {
    ...
    try {
      ...
      final breathing = await OfflineAISuggestionService.instance
          .getDailyBreathingSuggestion();
      final meditation = await OfflineAISuggestionService.instance
          .getDailyMeditationSuggestion();
      final festivals = await OfflineAISuggestionService.instance
          .getFestivalMessageSuggestions(count: 2);
      final tipEn = await OfflineAISuggestionService.instance
          .getEventPlanningSuggestion(hindi: false);
      final tipHi = await OfflineAISuggestionService.instance
          .getEventPlanningSuggestion(hindi: true);
      if (!mounted) return;
      setState(() {
        _modelsReady = true;
  ...
  Future<void> _loadFestivalHighlight() async {
    if (_loadingFestival) return;
    _loadingFestival = true;
    try {
  ...
  void _loadRecentPurchases() {
    try {
      ...
      final list = (box.get('history', defaultValue: <dynamic>[]) as List)
          ...
          .cast<Map<String, dynamic>>();
      setState(() {
        _recentPurchases = list.take(6).toList();
      });
    } catch (_) {}
  }

  Future<void> _recordPurchase(
      Map<String, dynamic> gift, int pointsUsed, double price) async {
    try {
      ...
      final raw =
          (box.get('history', defaultValue: <dynamic>[]) as List).toList();
      raw.insert(0, entry);
      while (raw.length > 25) {
        raw.removeLast();
      }
      ...
      setState(() {
        _recentPurchases =
            raw.take(6).cast<Map>().cast<Map<String, dynamic>>().toList();
      });
    } catch (_) {}
  }
    ...
    showDialog(
        ...
        builder: (_) => AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(
                      "${gift['emoji']}  ${isHi ? gift['titleHi'] : gift['title']}",
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Text(isHi ? sampleHi : sampleEn,
                      style: const TextStyle(height: 1.3)),
                  const SizedBox(height: 12),
                  Text(isHi ? 'द्विभाषी उदाहरण:' : 'Bilingual Sample:',
                  ...
                  Container(
                    ...
                    child: Text(isHi ? sampleEn : sampleHi,
                        style: const TextStyle(
                            fontSize: 11,
              ...
              actions: [
                TextButton(
                    ...
                    child: Text(isHi ? 'बंद करें' : 'Close')),
                ElevatedButton(
                    onPressed: () {
                      Navigator.pop(context);
                      _simulateGiftPurchase(gift, usePoints: true);
                    },
                    child: Text(isHi ? 'पॉइंट्स से अनलॉक' : 'Unlock w/ Points'))
              ],
            ));
  }
  ...
  Future<void> _shareGiftToken(Map<String, dynamic> gift) async {
    final token =
        await VirtualGiftShareService.instance.createTokenForGift(gift);
    if (!mounted) return;
    showDialog(
        context: context,
        builder: (_) => AlertDialog(
              ...
              content: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(_isHindi ? 'मैन्युअली भेजें:' : 'Send manually:',
                      style: const TextStyle(
                  ...
                  Text(
                      _isHindi
                          ? 'प्राप्तकर्ता इस कोड को "Redeem Gift" में दर्ज करेगा।'
                          : 'Recipient will enter this code in "Redeem Gift".',
                      style:
    ...
    showDialog(
        ...
        builder: (_) => AlertDialog(
              ...
              actions: [
                TextButton(
                    onPressed: () => Navigator.pop(context),
                    child: Text(_isHindi ? 'रद्द' : 'Cancel')),
                ElevatedButton(
                    onPressed: () async {
                      final code = ctl.text.trim().toUpperCase();
                      final data = await VirtualGiftShareService.instance
                          .redeemToken(code);
                      if (!mounted) return;
                      Navigator.pop(context);
                      showDialog(
                          ...
                          builder: (_) => AlertDialog(
                                ...
                                content: Text(data == null
                                    ? (_isHindi
                                        ? 'कोड सही नहीं है या पहले उपयोग हो चुका है।'
                                        : 'Code invalid or already used.')
                                    : "${data['emoji']}  ${_isHindi ? data['titleHi'] : data['title']}"),
                                actions: [
                                  TextButton(
                                      onPressed: () => Navigator.pop(context),
                                      child: Text(_isHindi ? 'ठीक' : 'OK'))
                                ],
                              ));
                    },
              ...
              content: Column(mainAxisSize: MainAxisSize.min, children: [
                TextField(
                    ...
                    decoration: const InputDecoration(
                        labelText: 'Category (UI, AI, Festival, Mood, Other)')),
                TextField(
                    controller: msgCtl,
                    ...
                    onPressed: () async {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content:
                                    Text('Feedback saved locally (offline)')));
                      }
                    },
  ...
  Future<void> _exportAnonymizedSummary() async {
    ...
    final map = {
      ...
      'festival_highlight': _festivalHighlight == null
          ? null
          : {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Anonymized summary copied to clipboard')));
  }

  Widget _buildFestivalHighlightBadge() {
    final h = _festivalHighlight;
        ...
        onTap: () {
          showDialog(
              ...
              builder: (_) => AlertDialog(
                    title: Text(h.name),
                    content: Text(
                        'Coming in ${h.daysAway} day${h.daysAway == 1 ? '' : 's'}\nPlan a thoughtful reconnect or greeting.'),
                    actions: [
                      TextButton(
                          onPressed: () => Navigator.pop(context),
  ...
  bool _isBundleUnlocked(String id) {
    try {
      final box = Hive.box('virtual_gift_purchases');
      final history =
          (box.get('history', defaultValue: <dynamic>[]) as List).cast<Map?>();
  ...
  void _purchaseBundle(Map<String, dynamic> bundle) async {
    ...
    if (_loyaltyPoints > 0) {
      final maxDiscount =
          (price * (LoyaltyPointsService.maxDiscountPercent / 100)).floor();
      pointsToUse = _loyaltyPoints.clamp(0, maxDiscount);
      final calc =
          LoyaltyPointsService.instance.calculateDiscount(price, pointsToUse);
    ...
    showDialog(
        ...
        builder: (_) => AlertDialog(
              ...
              content: Text(_isHindi
                  ? 'पैक तैयार है। ${(pointsToUse > 0) ? 'पॉइंट्स उपयोग: $pointsToUse' : ''}'
                  : 'Pack ready. ${(pointsToUse > 0) ? 'Points used: $pointsToUse' : ''}'),
              actions: [
                TextButton(
                    onPressed: () => Navigator.pop(context),
                    child: Text(_isHindi ? 'ठीक है' : 'OK'))
        ...
        children: [
          Row(
            children: [
              ...
              Text(_isHindi ? 'बंडल पैक' : 'Bundle Packs',
                  style: const TextStyle(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Expanded(
                            child: Text(_isHindi ? b['titleHi'] : b['title'],
                                style: const TextStyle(
                                    fontSize: 14,
                                    fontWeight: FontWeight.w600))),
                        Text(
                            '₹${(b['bundlePrice'] as double).toStringAsFixed(0)}',
                            style: const TextStyle(
                                fontSize: 12, fontWeight: FontWeight.bold)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Text(_isHindi ? b['descHi'] : b['desc'],
                        style: const TextStyle(fontSize: 11, height: 1.25)),
                    const SizedBox(height: 8),
                    Wrap(
                      spacing: 6,
                      children: (b['items'] as List)
                          .map((id) => Chip(
                              label: Text(id,
                    ...
                    Row(
                      children: [
                        ElevatedButton.icon(
                          ...
                          icon: Icon(unlocked ? Icons.check : Icons.lock_open,
                              size: 16),
                          label: Text(unlocked
                              ? (_isHindi ? 'अनलॉक' : 'Unlocked')
                              : (_isHindi ? 'अनलॉक करें' : 'Unlock')),
                        ),
                        const Spacer(),
                        if (!unlocked)
                          Text(_isHindi ? '15% तक पॉइंट्स' : 'Up to 15% points',
                              style: const TextStyle(
  ...
  void _simulateGiftPurchase(Map<String, dynamic> gift,
    ...
    if (usePoints && _loyaltyPoints > 0) {
      ...
      final maxDiscount =
          (original * (LoyaltyPointsService.maxDiscountPercent / 100)).floor();
      pointsToUse = _loyaltyPoints.clamp(0, maxDiscount);
      final calc = LoyaltyPointsService.instance
          .calculateDiscount(original, pointsToUse);
      if (calc.actualPointsToUse > 0) {
        await LoyaltyPointsService.instance.usePointsForPurchase(
          itemName: gift['id'],
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Text(_isHindi ? 'हाल की खरीद' : 'Recent Purchases',
                  style: const TextStyle(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                if (_recentPurchases.isNotEmpty || true)
                  const SizedBox(height: 24),
                ...
                ValueListenableBuilder<MetricsSnapshot>(
                  valueListenable:
                      MetricsAggregatorService.instance.snapshotNotifier,
                ...
                QuickActionsPanel(
                  ...
                  actions: [
                    QuickActionItem(
                      icon: Icons.psychology_alt,
                      labelEn: 'Chat with Dr. Iris',
                      labelHi: 'Dr. Iris से बात करें',
                      color: Colors.deepPurple,
                      onPressed: () => _navigateToDrIris(),
                    ),
                    QuickActionItem(
                      icon: Icons.psychology,
                      labelEn: 'Mood Check-in',
                      labelHi: 'मूड चेक-इन',
                      color: Colors.teal,
                ...
                Container(
                  ...
                  decoration: CoralTheme.translucentCard(
                      alpha: 0.18, radius: BorderRadius.circular(16)),
                  child: Column(
                    children: [
                      _buildStatRow(
                ...
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: CoralTheme.translucentCard(
                          alpha: 0.15, radius: BorderRadius.circular(12))
                      .copyWith(
                    border: Border.all(
                ...
                Container(
                  ...
                  decoration: CoralTheme.translucentCard(
                      ...
                      .copyWith(
                    border:
                        Border.all(color: Colors.green.withValues(alpha: 0.55)),
                  ),
                  child: Row(
                    children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    OutlinedButton.icon(
                        onPressed: _exportAnonymizedSummary,
      ...
      bottomNavigationBar:
          const BottomFlowNav(currentPageId: 'GiftMarketplacePage'),
    );
  }
            ...
            children: [
              ...
              if (_isDownloadingModels) ...[
                ...
                Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Text(_isHindi ? 'आकार ~300KB' : 'Size ~300KB',
                      style:
        ...
        child: ExpansionTile(
          ...
          title: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(_isHindi ? 'उपहार व बंडल' : 'Gifts & Bundles',
                  style: const TextStyle(
              ...
              if (recommended != null) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(12)),
                  child: Text(_isHindi ? 'सुझाव' : 'Suggested',
                      style: const TextStyle(
                          fontSize: 10,
                          fontWeight: FontWeight.w600,
                          color: Colors.orange)),
          ...
          children: [
            ...
            VirtualGiftsSection(
              ...
              onPurchase: (g, {required usePoints}) =>
                  _simulateGiftPurchase(g, usePoints: usePoints),
  ...
  void _showGiftsInfo() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title:
            Text(_isHindi ? 'उपहार कैसे काम करते हैं?' : 'How do gifts work?'),
  ...
  String _platformDownloadLabel() {
    ...
    switch (Theme.of(context).platform) {
      case TargetPlatform.android:
        return _isHindi ? 'एंड्रॉइड मॉडल डाउनलोड' : 'Download Android Models';
      case TargetPlatform.iOS:
        return _isHindi ? 'iOS मॉडल डाउनलोड' : 'Download iOS Models';
      case TargetPlatform.macOS:
        return _isHindi ? 'macOS मॉडल डाउनलोड' : 'Download macOS Models';
      case TargetPlatform.windows:
        return _isHindi ? 'Windows मॉडल डाउनलोड' : 'Download Windows Models';
      case TargetPlatform.linux:
        return _isHindi ? 'Linux मॉडल डाउनलोड' : 'Download Linux Models';
      case TargetPlatform.fuchsia:
        return _isHindi ? 'मॉडल डाउनलोड' : 'Download Models';
    }
  }
  ...
  Future<void> _performModelDownload() async {
    ...
    try {
      ...
      for (int i = 1; i <= steps; i++) {
        ...
        if (!mounted) return;
        setState(() => _modelDownloadProgress = i / steps);
      }
      if (phone != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_isHindi
              ? 'मॉडल तैयार. ऑफ़लाइन AI सक्रिय.'
              : 'Models ready. Offline AI active.'),
  ...
  Widget _buildDrIrisSection() {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.all(20),
        decoration: CoralTheme.translucentCard(
            alpha: 0.20, radius: BorderRadius.circular(16)),
        child: Row(
          children: [
            const CircleAvatar(
              radius: 30,
  ...
  void _navigateToFeature(Map<String, dynamic> feature) {
    if (feature['title'] == 'Dr. Iris Emotional Therapist') {
    ...
    } else if (feature['title'] == 'Breathing Exercises' ||
        feature['title'] == 'Meditation Guide') {
      Navigator.push(
        context,
        MaterialPageRoute(
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              const DailyProgressPage(), // Updated to new sample page
        ),
      );
  ...
  bool _hasReconnectCandidate() {
    try {
      ...
      for (final c in box.values) {
        if (c.daysSinceLastContact > 60 ||
            c.metadata['pending_reconnect'] == true) {
          return true;
        }
      }
    } catch (_) {}
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _metricChip(_isHindi ? 'औसत मूड' : 'Avg Mood',
                  snap.avgMood7d.toStringAsFixed(1)),
              _metricChip(_isHindi ? 'चेक-इन्स' : 'Check-ins',
                  snap.checkIns7d.toString()),
              _metricChip(_isHindi ? 'सांस' : 'Breathing',
                  snap.breathingSessions7d.toString()),
              _metricChip(_isHindi ? 'ध्यान' : 'Meditation',
                  snap.meditationSessions7d.toString()),
              _metricChip(_isHindi ? 'नींद (घं)' : 'Sleep (h)',
                  snap.sleepAvgHours7d.toStringAsFixed(1)),
              _metricChip(_isHindi ? 'रीकनेक्ट' : 'Reconnects',
                  snap.reconnects30d.toString()),
              _metricChip(_isHindi ? 'समाधान' : 'Repairs',
                  snap.conflictRepairs30d.toString()),
            ],
          ),
          const SizedBox(height: 6),
          Text(
  ...
  Widget _buildUnifiedAICard() {
    ...
    return ValueListenableBuilder<Map<String, String>>(
      ...
      builder: (context, insights, _) {
        final hasSuggestionData = _modelsReady &&
            (_breathingSuggestion != null ||
                _meditationSuggestion != null ||
                _festivalMessages.isNotEmpty ||
                _eventPlanningTipEn != null);
        ...
        return Container(
            ...
            decoration: BoxDecoration(
                gradient: const LinearGradient(
                    colors: [Color(0xFFF5F4FF), Color(0xFFEDE7F6)]),
                borderRadius: BorderRadius.circular(20),
                boxShadow: CoralTheme.glowShadow(0.10),
                border: Border.all(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (lastRefresh != null)
                      Text(
                        _isHindi
                            ? _relativeTimeHi(lastRefresh)
                            : _relativeTimeEn(lastRefresh),
                        style: const TextStyle(
                            fontSize: 10, color: Colors.black54),
                      ),
                    IconButton(
                ...
                if (hasInsights) ...[
                  Text(_isHindi ? 'दैनिक पैटर्न' : 'Daily Patterns',
                      style: const TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w600,
                  ...
                  const SizedBox(height: 10),
                  ...orderedInsightKeys
                      .map((k) => _insightLine(k, insights[k] ?? '')),
                  const SizedBox(height: 6),
                  Text(
                    _isHindi
                        ? 'सभी विश्लेषण निजी रूप से ऑफ़लाइन उत्पन्न।'
                        : 'All analysis generated privately offline.',
                    style: const TextStyle(
                        fontSize: 10,
                        color: Colors.black54,
                        fontStyle: FontStyle.italic),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(body,
              style: const TextStyle(
                  fontSize: 11, height: 1.2, color: Colors.black87),
              maxLines: 3,
              overflow: TextOverflow.ellipsis)
        ],
      ),
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          ...
          value,
          style:
              const TextStyle(fontSize: 11, height: 1.25, color: Colors.black),
        )),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
            child: Text(
          _isHindi
              ? 'किसी पुराने मित्र से दोबारा जुड़ने का अच्छा समय'
              : 'Good time to reconnect with an old friend',
          style: const TextStyle(fontSize: 11, color: Colors.black87),
        )),
        TextButton(
          onPressed: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => const RelationshipPulsePage()));
          },
